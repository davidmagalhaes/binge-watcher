...
class EpisodeActivityTest {
    ...
    private var activityScenario : ActivityScenario<EpisodeActivity>? = null

    @Before
    fun before(){
        EspressoIdlingResource.clearCounting()
        IdlingRegistry.getInstance().register(EspressoIdlingResource.counting)
        activityScenario = ActivityScenario.launch(EpisodeActivity::class.java)
    }
